...
@property (readwrite, copy, nullable, nonatomic) NSString *name API_AVAILABLE(ios(13.0), macos(11.0), watchos(6.0));
@property (readwrite, copy, nullable, nonatomic) NSString *bundleIdentifier API_AVAILABLE(ios(13.0), macos(11.0), watchos(6.0));
@property (readwrite, copy, nullable, nonatomic) NSString *path API_AVAILABLE(ios(13.0), macos(11.0), watchos(6.0));
...
+ (instancetype)successWithResolvedApp:(App *)resolvedObject NS_SWIFT_NAME(success(with:));

// This resolution result is to ask Siri to disambiguate between the provided App.
+ (instancetype)disambiguationWithAppsToDisambiguate:(NSArray<App *> *)objectsToDisambiguate NS_SWIFT_NAME(disambiguation(with:));

// This resolution result is to ask Siri to confirm if this is the value with which the user wants to continue.
+ (instancetype)confirmationRequiredWithAppToConfirm:(App *)objectToConfirm NS_SWIFT_NAME(confirmationRequired(with:));
